...
var _jackpot = require('jackpot');

var _jackpot2 = _interopRequireDefault(_jackpot);

var _apiDescription = require('./api-description');
...
function gbkEncode(string) {
  return _iconvLite2.default.encode(string, 'gbk');
}
...
function gbkDecode(buffer) {
  return _iconvLite2.default.decode(buffer, 'gbk');
}
...
class RegulatoryMessage {
   ...
   */
  constructor(clientConfig, clientLogId, functionCode, paramsList, ignoreWebSign) {
    ...
    if (!_apiDescription.api.request.hasOwnProperty(functionCode)) {
  ...
  composeMessageBody() {
    ...
    let extract = (keyObject, dataObject) => {
      ...
      if (keyObject.required && !dataObject.hasOwnProperty(keyObject.key) && !keyObject.hasOwnProperty('default')) {
        throw new Error('[JZB] Missing key ' + keyObject.key + ' for function ' + self._functionCode);
      }
      ...
      if (keyObject.type === Number && !/^\d+$/.test(value)) {
        throw new Error('[JZB] Incorrect key ' + keyObject.key + ' format for function ' + self._functionCode);
      }
      if (value.toString().length > keyObject.length) {
        throw new Error('[JZB] Key ' + keyObject.key + ' overflow for function ' + self._functionCode);
      }
      if (keyObject.allowedValues && keyObject.allowedValues.indexOf(value) === -1) {
        throw new Error('[JZB] Key ' + keyObject.key + ' has invalid value for ' + self._functionCode);
      }
      return value;
    };
    let keyDictionary = _apiDescription.api.request[this._functionCode].keys;
    let messageBody = '';
  ...
  composeMessageHead() {
    ...
    messageHead += (0, _dateformat2.default)(new Date(), this._clientConfig.dateTimeFormat);
  ...
  composeNetworkHead() {
    let networkHead = '';
    let messageLength = this.messageBodyBuffer.length + Buffer.from(this.messageHead).length;
    ...
    networkHead += (0, _dateformat2.default)(new Date(), this._clientConfig.dateTimeFormat);
  ...
  get buffer() {
    ...
    const totalLength = networkHeadBuffer.length + messageHeadBuffer.length + this.messageBodyBuffer.length;

    return Buffer.concat([networkHeadBuffer, messageHeadBuffer, this.messageBodyBuffer], totalLength);
  }
}
...
class RegulatoryResponse {
  constructor(responseBuffer) {
    ...
    if (this._responseCode !== '000000') {
      throw new Error('[JZB] Invalid Response Code [' + this._responseCode + ']: ' + this._responseMessage);
    }
  ...
  parseResponseBody() {
    ...
    let keyDictionary = _apiDescription.api.response[this._functionCode].keys;
...
class RegulatoryClient {
  constructor(config) {
    if (!('port' in config && 'server' in config && 'marketId' in config && 'webServiceHost' in config)) {
      throw new Error('[JZB] Cannot initialize retulatory client.');
    }
    // Set up Regulatory Client (见证宝)
    this._pool = new _jackpot2.default(10, () => {
      return _net2.default.connect(config.port, config.server);
    }, {
      min: 100,
      max: 30000,
      retries: 3
    ...
    this._clientConfig.webEndpoint = joinUrl(this._clientConfig.webServiceHost, this._clientConfig.webPath);
  }

  sendMessage(clientLogId, functionCode, paramsList, callback) {
    let message = new RegulatoryMessage(this._clientConfig, clientLogId, functionCode, paramsList, false);
  ...
  preparePasswordForm(clientLogId, functionCode, paramsList, formParamsList) {
    let form = '<form name="payment-form' + clientLogId + '" ' + 'action=' + this._clientConfig.webEndpoint + '" method="post">';

    // Put together keys
    for (let keyObject of _apiDescription.api.paymentForm.keys) {
      let value;
      if (keyObject.key === 'orderid') {
        ...
        if (formParamsList.type === 'V') {
          let message = new RegulatoryMessage(this._clientConfig, clientLogId, functionCode, paramsList, true);
        ...
        // Throws Error for missing required param;
        if (keyObject.required && !formParamsList.hasOwnProperty(keyObject.key) && !keyObject.hasOwnProperty('default')) {
          throw new Error('[JZB] Missing key ' + keyObject.key + ' for payment form for ' + functionCode);
        }
        // Writes default value for non-existing default;
        if (keyObject.hasOwnProperty('default') && !formParamsList.hasOwnProperty(keyObject.key)) {
      ...
      // Validation
      if (keyObject.type === Number && !/^\d+$/.test(value)) {
        throw new Error('[JZB] Incorrect key ' + keyObject.key + ' format for payment form ' + functionCode);
      }
      if (value.toString.length > keyObject.length) {
        throw new Error('[JZB] Key ' + keyObject.key + ' overflow for function ' + functionCode);
      }
      if (keyObject.allowedValues && keyObject.allowedValues.indexOf(value) === -1) {
        throw new Error('[JZB] Key ' + keyObject.key + ' has invalid value for function ' + functionCode);
      }

      // Put forms together
      form += '<input type="hidden" name="' + keyObject.key + '" value="' + value + '" />';
    }
...
exports.default = RegulatoryClient;